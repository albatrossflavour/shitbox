...
Builds an MBTiles SQLite file of OpenStreetMap standard tiles covering a
+/-20 km corridor around the configured rally waypoints. Designed to run
once on the laptop before the rally (over fast home internet), then ship
the resulting .mbtiles file to the Pi so the dashboard can serve map tiles
entirely offline.
...
# MapTiler raster tiles — requires an API key passed via --maptiler-key.
# Style `openstreetmap` reproduces the classic OSM carto look.
TILE_URL_TEMPLATE = (
    "https://api.maptiler.com/maps/openstreetmap/256/{z}/{x}/{y}.png?key={key}"
)
RATE_LIMIT_SECONDS = 0.05  # 20 req/s — MapTiler paid/free tier handles this easily
...
def fetch_and_store(
    ...
    for z, x, y in tiles:
        ...
        if already_present(mbtiles_path, z, x, tms_y):
            skipped += 1
            continue
        url = TILE_URL_TEMPLATE.format(z=z, x=x, y=y, key=api_key)
        try:
...
def main() -> None:
    ...
    p.add_argument("--maptiler-key", required=True, help="MapTiler API key")
    args = p.parse_args()
    ...
    fetch_and_store(tiles, Path(args.out), args.maptiler_key)
