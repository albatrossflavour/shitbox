...
`~/Brain/projects/shitbox-rally-2026.md` is the source of truth for project state, active work, and hardware context. Update the "Active Now" section whenever significant work is completed or a new investigation thread opens. For complex debugging sessions that may span multiple sessions, use `/gsd:debug`.
...
- The site reads `events.json` at `/captures/events.json`, generated by `EventStorage.generate_events_json()` — check the source for current fields
