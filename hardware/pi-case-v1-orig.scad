...
GX12_D          = 12.5;
SMA_HOLE_D      = 6.7;
SMA_X_OFF       = 40;   // X offset from FLANGE_W along the back wall (between left GX12 at 15 and portcullis at ~47)
CONN_TOP_MARGIN = 10;   // mm below the lid plate to the connector hole centres — GX12 bodies extend
                        // ~15-20 mm into the case so keep this small enough they hang into clear space
...
// ---- Clamshell geometry ----
SPLIT_Z    = 30;
INNER_H    = 93;   // doubled top half: was 60 (32.5 mm top wall), now 93 (65.5 mm top wall)
...
module pi_case_cable_slots_top() {
    ...
    //
    // SMA panel-mount at SMA_X_OFF (between left GX12 and portcullis
    // edge) at the same wall height — GPS antenna coax exits here
    // rather than threading through the portcullis vent gaps.
    ...
    // SMA panel-mount hole for GPS antenna coax
    translate([FLANGE_W + SMA_X_OFF,
               OUTER_D + 0.1,
               conn_z])
        rotate([90, 0, 0])
