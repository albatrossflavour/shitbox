...
|---------|------|------|-------------|-----------|-------------------|-------------|--------|
| 18-xx-01 | batch_sync | 1 | WEB-04 | unit | `pytest tests/sync/test_batch_sync.py -x -q` | ❌ W0 | ⬜ pending |
| 18-xx-02 | batch_sync | 1 | WEB-04 | unit | `pytest tests/sync/test_batch_sync.py -x -q` | ❌ W0 | ⬜ pending |
...
- [ ] `tests/sync/test_batch_sync.py` — stubs for batch_sync metric additions (shitbox_lux, shitbox_temp probe label)
...
| Nginx serves new JSON files without stale cache | WEB-01–04 | Requires live request inspection | curl -I https://shit-of-theseus.com/captures/notes.json and confirm Cache-Control is no-cache or short TTL |
