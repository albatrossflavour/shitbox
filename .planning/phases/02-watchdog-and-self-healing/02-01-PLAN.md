...
must_haves:
  truths:
    - "Systemd service unit has WatchdogSec=14 and StartLimitIntervalSec=0"
  ...
  artifacts:
    - path: "systemd/shitbox-telemetry.service"
      provides: "Hardened systemd unit with WatchdogSec=14 and StartLimitIntervalSec=0"
      contains: "WatchdogSec=14"
  ...
  key_links:
    ...
    - from: "systemd/shitbox-telemetry.service"
      ...
      via: "WatchdogSec=14 matches existing _notify_systemd WATCHDOG=1 petting"
      pattern: "WatchdogSec=14"
---

<objective>
Harden the systemd service unit for unlimited restarts with a 14-second watchdog timeout,
...
1. Change `WatchdogSec=30` to `WatchdogSec=14` (matches RuntimeWatchdogSec=14 that will be
...
4. Update the comment above WatchdogSec to reflect the new 14s timeout
...
All new functions must have full type annotations and structlog-compatible docstrings.
  ...
  <done>
    systemd unit has WatchdogSec=14 and StartLimitIntervalSec=0; buzzer.py exports
...
1. `test_watchdog_unit_file_has_14s`: Read `systemd/shitbox-telemetry.service`, assert
   `WatchdogSec=14` is present in the `[Service]` section
...
- `grep -c "WatchdogSec=14" systemd/shitbox-telemetry.service` returns 1
...
<success_criteria>
- Systemd unit file hardened with WatchdogSec=14 and StartLimitIntervalSec=0
